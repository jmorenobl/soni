...
class DialogueUnderstanding(dspy.Signature):
    ...
    dialogue_history = dspy.InputField(
        desc="Previous conversation context as a string",
        default=""
    )
    current_slots = dspy.InputField(
        desc="Currently filled slots as a JSON string",
        default="{}"
    )
    ...
    current_flow = dspy.InputField(
        desc="Current dialogue flow name",
        default="none"
    )
    ...
    reasoning = dspy.OutputField(
        desc="Brief reasoning for the extraction"
    )
