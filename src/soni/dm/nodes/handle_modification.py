...
async def handle_modification_node(
    ...
    if not nlu_result:
        logger.warning("No NLU result in state for handle_modification_node")
        return {"conversation_state": "error"}

    slots = nlu_result.get("slots", [])
    if not slots:
        logger.warning("No slots in NLU result for modification")
    ...
    # Get first slot (modifications typically have one slot)
    slot = slots[0]
    ...
    try:
        ...
        logger.error(f"Normalization failed for modification: {e}")
        return {"conversation_state": "error"}
    ...
    # Determine which step to return to (reuse logic from handle_correction)
    ...
    if flow_config:
        ...
        logger.debug(
            f"Modification check: required_slots={required_slots}, "
    ...
    if target_step:
        ...
        if target_step_config:
            ...
            if target_step_config.type == "confirm":
                ...
                # This handles the case where we modified a slot but need to continue collecting
            ...
            logger.info(
                f"Modification detected: returning to step '{target_step}' "
    ...
    logger.warning("Could not determine target step for modification, staying in current state")
    # Get acknowledgment message using template (Task 005)
    config = runtime.context["config"]
    acknowledgment = _get_response_template(
        config,
