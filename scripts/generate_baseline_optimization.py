...
from soni.core.commands import Command  # noqa: E402
...
def validate_command(expected: Command, actual: Command) -> bool:
    """Validate if actual command matches expected.

    This is the metric function used during optimization.

    Args:
        expected: Expected command from dataset
        actual: Command generated by the model
    ...
    # For commands with specific fields, check them
    # This is a simple type-based check; can be extended for field validation
    return True
...
def main(auto: str = "light", examples_per_combination: int = 3) -> int:
    ...
    metric = create_metric(validate_command)
