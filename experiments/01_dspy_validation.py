...
class DialogueUnderstanding(dspy.Signature):
    ...
    structured_command = dspy.OutputField(desc="User's intent/command (e.g., book_flight, cancel, help)")
...
class SimpleNLU(dspy.Module):
    ...
    def forward(self, user_message: str, dialogue_history: str = "", 
                current_slots: str = "{}", available_actions: str = "[]",
...
def create_trainset() -> list:
    ...
    trainset = [
        ...
        ).with_inputs("user_message", "dialogue_history", "current_slots", 
                     "available_actions", "current_flow"),
    ...
    additional_examples = [
        ...
        ("Book flight to Dubai tomorrow", "book_flight", '{"destination": "Dubai", "date": "tomorrow"}'),
...
def intent_accuracy_metric(example, prediction, trace=None) -> float:  # noqa: ARG001
    ...
    try:
        ...
        try:
            ...
            pred_slots = json.loads(prediction.extracted_slots) if prediction.extracted_slots else {}
...
def main():
    ...
    try:
        ...
        if optimization_time > 600:  # 10 minutes
            print(f"   ⚠️  ADVERTENCIA: Tiempo de optimización ({optimization_time:.2f}s) excede 10 minutos")
        else:
            print(f"   ✓ Tiempo dentro del umbral (< 10 minutos)")
    except Exception as e:
    ...
    if optimized_nlu:
        ...
        improvement_pct = (improvement / baseline_results['average_score'] * 100) if baseline_results['average_score'] > 0 else 0
        ...
        print(f"✓ Tiempo de optimización: {optimization_time:.2f}s ({'< 10 min' if optimization_time < 600 else '≥ 10 min'})")
        ...
        results = {
            ...
            "criteria_met": {
                ...
                "improvement_ge_5pct": improvement_pct >= 5.0,  # Usar porcentaje para comparación precisa
